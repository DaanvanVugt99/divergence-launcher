...
export const PatchStatusScreen = ({
  ...
  const canPatch = Boolean(
    selectedRomPath && sourceVerified && status?.patchPlan.status === 'ready',
  );
  ...
  return (
    <Card>
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            ...
            <CardDescription>
              Verify the source ROM, apply the Divergence patch, and validate the patched output.
            </CardDescription>
          </div>
          <Badge variant={status?.patchPlan.status === 'ready' ? 'default' : 'destructive'}>
            {statusLabel(status)}
          </Badge>
        </div>
      </CardHeader>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          <div className="rounded-md border bg-card p-4">
            ...
            <div className="mt-2 break-words text-sm text-muted-foreground">
              {formatPathForWrap(selectedRomPath, 'No ROM selected')}
            </div>
            <div className="mt-3 flex flex-wrap items-center gap-2">
              <Badge
                variant={sourceVerified ? 'default' : verification ? 'destructive' : 'secondary'}
              >
              ...
              <Button
                ...
                variant="outline"
                onClick={onVerifyRom}
                disabled={!selectedRomPath || isPatching}
              >
                Verify
              </Button>
            </div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <div>
            ...
            <div className="text-sm text-muted-foreground">
              {status?.patchPlan.patchVersion ?? 'v0.1'}
            </div>
          </div>
          <div>
            <div className="text-sm font-medium">Expected source</div>
            <div className="text-sm text-muted-foreground">
              {status?.patchPlan.expectedBaseRoms[0]?.label ?? 'Not configured'}
            </div>
          </div>
          <div>
            <div className="text-sm font-medium">Output file</div>
            <div className="text-sm text-muted-foreground">
              {status?.patchPlan.outputFileName ?? 'Not set'}
            </div>
          </div>
        </div>
      </CardContent>
      <CardFooter className="justify-end gap-3">
        <Button
          type="button"
          variant={patchApplied ? 'outline' : 'default'}
          onClick={onPatchRom}
          disabled={!canPatch || isPatching}
          className="gap-2"
        >
